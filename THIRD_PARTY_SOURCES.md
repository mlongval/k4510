...
| | |
|---|---|
| Role | The machine's own keyboard layouts (F7 -> Input -> Keyboard layout), the same XKB data the K4510 Linux's consoles use. |
...
| | |
|---|---|
| Role | The one screen font: 8x8 (the 240-line modes, at `$010000`) and 8x16 (640x480, at `$010800`); also the F7 menu's and side panel's own font, which must draw when the guest has wrecked everything. |
