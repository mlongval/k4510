...
\item \textbf{CPU: the 45GS10} --- the MEGA65's 45GS02 instruction set (a
  ...
  under Machine --- ten steps from 202.5\,MHz down to 10). A host nobody
...
froze. The menu draws with its own font and never touches the machine's
...
\begin{description}
\item[Video] the border; the screen font (a live swap of the character
  set --- \emph{kernel8}, \emph{unscii}, \emph{open-roms} and
  \emph{PXLfont}, twelve of Damien Guard's ZX~Origins faces, and
  \emph{C64 chargen} if you have put a converted Commodore character ROM
  in \pth{/SYSTEM/ETC/chargen.bin}, which \pth{/SYSTEM/ETC/README.TXT}
  explains); \emph{resolution} and \emph{left/top margin}, which are the
  \verb|MODE| command's two knobs; \emph{scanlines}, a dark line between
  each of the machine's, drawn exactly rather than overlaid;
  \emph{scaling}: \emph{sharp}, \emph{soft}, or \emph{sharp-fit} (hard
  pixels \emph{and} a whole-number multiple, so every pixel is the same
  size on the glass); full screen; \emph{vertical sync}, off to begin
...
\item[Terminal] \emph{status bands} and how many rows each takes
...
\item[Input] the reset chord; which key opens the menu; whether a click
  ...
  the picture; and the keyboard, above.
\item[Machine] \emph{save state} and \emph{load state}, four slots each
  ...
  reset; power cycle; stop the Tube; quit; \emph{CPU clock} --- the ten
  steps, live; choosing one switches \emph{Auto clock} off, because a
...
\item[Host] on the K4510's own Linux only: the computer's name, its
  network address and Tailscale's, a \emph{Wi-Fi / network setup} row
  that opens the network settings on a spare console, and a row that
  telnets into the Linux for you.
\end{description}
