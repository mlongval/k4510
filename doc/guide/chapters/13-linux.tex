...
JIM also draws \emph{pictures}. It speaks the Kitty terminal's graphics
protocol, so a program that can show an image in a terminal that way shows it
here, among the text, scrolling with it:
...
\noindent The picture goes onto VICKY's top layer, dithered onto 216 of the
palette's colours (entries 40 to 255; the console's sixteen are left alone),
and is gone when the screen is cleared. Raw RGB and PNG are understood, sent
in the escape itself or named as a file; animation and pictures \emph{under}
the text are not. \verb|BOOK| uses the same door for the pictures in its
pages, and so can any program of the machine's own: what it names is a file
...
The third door is a \emph{login} on the Linux beneath --- its own tty,
its own session --- where \verb|!| gives a shell on the Tube. It is F12
$\rightarrow$ Host $\rightarrow$ \emph{Telnet into the host}, and it
asks for no password: the telnet it speaks to listens on the loopback and
nowhere else, so the only way to knock on it is from the machine standing
on that very Linux --- which already opens an unauthenticated shell there
with \verb|!|. One line in \pth{/STARTUP.BAT} gives it a name, which is
how the shipped \pth{/SYSTEM/ETC/STARTUP.SAMPLE} has it:

\begin{type}
ALIAS HOST TELNET 127.0.0.1 23
\end{type}

\noindent \verb|linux = locked| in the menu file shuts all of them
together.
...
takes 4\,GB off its end for a second partition, \texttt{K4510LIVE}, and
copies the system onto \emph{that}; arranges for your settings and files
to be kept on \texttt{K4510}; and adds two entries to the computer's boot
menu. There are two partitions because the machine loads itself into
memory when it starts, and what it loads is the whole of the partition
the system is on: kept apart, your files stay on the disk and only the
system is copied. (A \texttt{K4510} partition under 8\,GB is left as one,
and says so; 8\,GB or more is the size to make.) The two entries:
