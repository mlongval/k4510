...
F9, Ctrl-F9 & save every changed file, compile the \texttt{.C} (\verb|CC|) or \texttt{.PAS} (\verb|PAS|); and run it \\
...
\begin{aside}
\textbf{Where it is going.} This is PROG's second stage. Next come
projects --- a \texttt{PROJECT.K4P} naming a program's sources, so F9
builds the program, not the file in front, and a C program can be several
files --- then the mouse and selection, and last the interpreters:
EhBASIC, Microsoft BASIC, LOGO and Forth run on the file in front of you,
their errors in the same list.
\end{aside}
