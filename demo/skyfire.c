...
static void hud(void)
{
    ...
    put_str(27, 0, "WAVE "); put_dec(32, 0, wave, 2);
...
static uint8_t dive_gap(void) { uint8_t g = (uint8_t)(100 - wave * 10); return g < 30 ? 30 : g; }
static uint8_t max_divers(void) { uint8_t m = (uint8_t)(1 + wave / 2); return m > 3 ? 3 : m; }
...
void main(void)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if ((frame & 1) == 0) { fx += sway * 4; if (fx >= 80 * 4) sway = -1; if (fx <= 0) sway = 1; }
            if (!dead_timer) {
