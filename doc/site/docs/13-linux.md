...
`\n` is Enter and `~` a half-second pause; keys without a letter go by name (`up`, `pgdn`, `esc`, `f1`…). Typing and a screenshot at a time is the machine driven from another room — F7’s menu included, with `--key f7`. F7 → Input → *Key pipe* can turn it off, or — as it starts — show every key so typed at the foot of the screen.
