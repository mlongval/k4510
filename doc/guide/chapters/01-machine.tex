...
\item \textbf{CPU: the 45GS10} --- the MEGA65's 45GS02 instruction set (a
  ...
  fantasy and its timings are suggestions. The clock is a setting (F12,
...
\item \textbf{Video: VICKY} --- 640$\times$480, 256 colours from a 24-bit
  ...
  480 lines however few of them the picture uses. The 640$\times$480
  screen comes two ways in the menu: \emph{640x480}, eighty columns by
  thirty in tall 8$\times$16 letters, and \emph{640x480x60}, eighty by
  sixty in 8$\times$8 --- the screen the machine had before it settled on
  one font. \verb|MODE 0 60| and \verb|MODE 0 30| choose them at the
  prompt, and the machine remembers which.
\item \textbf{Sound: MELODY} --- an OPL2, the Yamaha YM3812, at
  ...
  Pascal's \verb|Sound| use. Beside it at \texttt{\$D4C0} is a
  \textbf{DigiMAX}: four 8-bit DACs for sampled sound, with nothing between
  a program and the loudspeaker but the register --- \texttt{\$80} is
  silence, and a sound is that number changing quickly. DOOM's gunfire
  comes out of DAC~0. A floating-point MATH unit sits beside it,
...
\item \textbf{K/OS}: a shell with directories, in ROM --- and on the disk,
  one word away each, a shelf of languages: EhBASIC with graphics and
  sprites,\shipif{lang:MSBASIC}{ Microsoft's own BASIC of 1977,} BBC BASIC on
  the Tube, Forth, LOGO, CP/M on a Z80, and RX, the machine's REXX. Two compilers are one word away
...
and PageUp) --- one key cannot do it by accident. F12 opens the settings
...
The keyboard is the one in front of you, and F12~$\rightarrow$~Input
...
\section{The F12 menu}
F12 freezes the machine and
...
closes at the categories; F12 closes from anywhere. The mouse works too.
Shift+F12 pauses the machine instead, the picture held; Shift+F12 again
goes on. The menu was on F7 until the autumn of 2026, a habit from the
Commodore's keyboard; most emulators keep theirs on F12, and F7 and F8
are ordinary keys now, for the programs that want all ten. Input
$\rightarrow$ \emph{Menu key} puts it back on F7 for fingers that
remember.
...
\screen{shots/menu.png}{The F12 menu: categories on the left, the Video
page on the right.}
...
\item[Shell] whether an unknown word at the prompt may run a CP/M
  \texttt{.COM}\shipif{chapter:09-cpm}{ (Chapter~\ref{cha:cpm})} --- off to begin with, on
...
\section{Placement, the register panel, and F8}
...
instead of in the middle, and the \emph{Sidebars} row fills what is left.
Its choices are the sidebars in \pth{/SYSTEM/SIDEBARS}, one zip each: the
border colour, a gradient, a knot, scenes --- and \emph{registers}, the
machine as the emulator sees it. A register panel beside a centred
picture makes no sense, so choosing it puts the picture left.

Below it, \emph{Edit options\ldots} opens the chosen sidebar's own
options in VI: \pth{/SYSTEM/SIDEBARS/ANTFARM/OPTIONS.CFG} for the ant farm
--- its \texttt{speed}, and the length of its day. Save the file and the
sidebar changes within a second. \pth{/SYSTEM/SIDEBARS/SIDEBARS.CFG} holds
what is for all of them: another sidebar on the right
(\texttt{right = tetris}), a new one every so often (\texttt{change = 1h}),
and whether Halloween and Christmas keep to their months. The ant colony
is kept across a power cycle, in the same folder.
