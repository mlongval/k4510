...
int main(void)
{
    static const char *order[] = { "border", "gradient", "knot", "registers", "halloween", "christmas", "space", "river", "dreamfall", "tetris", "antfarm", "matrix", "doom", "navidrome" };
    ...
    { char b[32]; load("video.sidebars = tetris\n"); settings_step(SET_VIDEO_SIDEBARS, 1);
      ...
      settings_step(SET_VIDEO_SIDEBARS, 1);
      CHECK(!strcmp(settings_text(SET_VIDEO_SIDEBARS, b, sizeof b), "matrix"), "then matrix (%s)", settings_text(SET_VIDEO_SIDEBARS, b, sizeof b));
      settings_step(SET_VIDEO_SIDEBARS, 1);
      CHECK(!strcmp(settings_text(SET_VIDEO_SIDEBARS, b, sizeof b), "doom"), "then doom (%s)", settings_text(SET_VIDEO_SIDEBARS, b, sizeof b));
      settings_step(SET_VIDEO_SIDEBARS, 1);
      CHECK(!strcmp(settings_text(SET_VIDEO_SIDEBARS, b, sizeof b), "navidrome"), "then navidrome, the last of them (%s)", settings_text(SET_VIDEO_SIDEBARS, b, sizeof b));
      settings_step(SET_VIDEO_SIDEBARS, 1);
    ...
    { static uint32_t px[240 * 1080]; static const char *nm[] = { "halloween", "christmas", "space", "river", "dreamfall", "tetris", "antfarm", "matrix", "doom", "navidrome" };
      for (int s = 0; s < SAVER_COUNT; s++) {
    ...
    { char gp[800]; snprintf(gp, sizeof gp, "%s/SYSTEM/SIDEBARS/SIDEBARS.CFG", tmp);
      ...
      { unsigned seen = 0; for (long t = 0; t < 40 * 600; t += 600) seen |= 1u << sidebars_builtin(sidebars_shown(af, 0, t, 7));
        CHECK(!(seen & (1u << SIDEBAR_REGISTERS)) && !(seen & (1u << SIDEBAR_HALLOWEEN)) && !(seen & (1u << SIDEBAR_CHRISTMAS)) && !(seen & (1u << SIDEBAR_DOOM)) && !(seen & (1u << SIDEBAR_NAVIDROME)) && __builtin_popcount(seen) == SIDEBAR_COUNT - 5,
              "July, every 10 minutes: the %d that are not seasonal, the panel, a gamebar or the radio (%x)", SIDEBAR_COUNT - 5, seen);
