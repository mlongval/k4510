...
\verb|HOST| is the third door: a \emph{login} on the Linux beneath ---
its own tty, its own session --- where \verb|!| gives a shell on the
Tube. It is the same thing as F12 $\rightarrow$ Host $\rightarrow$
\emph{Telnet into the host}, in one word. The telnet it speaks to listens
on the loopback and nowhere else, and it asks for no password: the only
way to knock on it is from the machine standing on that very Linux, and
that machine already opens an unauthenticated shell there with
\verb|!|. \verb|linux = locked| in the menu file shuts all three
together.
...
(Section~\ref{sec:menufile}) turns \verb|!|, \verb|SSH|, \verb|HOST| and
the menu's telnet row away with a message, and \verb|consoles = locked| makes the
