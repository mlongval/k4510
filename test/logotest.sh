...
K4510_DUMP=200000,$(printf '%x' $((640*480+0x4010))) ./test/headless rom/kernal.bin 'CD /LANG/LOGO
~LOGO
~~PRINT 2 + 3 * 4
~TO SQ :N
~REPEAT 4 [FD :N RT 90]
...
W,H=640,480
if len(b) < W*H+0x4010: print("logotest: FAILED: short dump", len(b)); sys.exit(1)
...
# the turtle: sprite 0 enabled, 8 bpp, Z 3, 32x32 (TURTLE.SPR's frames, 16 KB after the bitmap, the
# attribute table after them -- the test read the old 16x16 layout until 2026-09-15); its frame has
# pixels; it sits over (350,210)
attr=b[W*H+0x4000:W*H+0x4010]; a=int.from_bytes(attr[4:8],'little')-0x200000
glyph=b[a:a+1024] if 0 <= a <= len(b)-1024 else b''
...
ok = ok and attr[8] == 0x33 and attr[9] == 0x0A and x == 350-16 and y == 210-16 and sum(1 for v in glyph if v) >= 20
