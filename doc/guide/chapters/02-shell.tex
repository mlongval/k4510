...
\texttt{/CPM} & the Z80's drives, \texttt{A} to \texttt{P}, one folder each\shipif{chapter:09-cpm}{ (Chapter~\ref{cha:cpm})} \\
...
\verb|RUN cube.prg| runs a program; so does \verb|RUN cube|, and so does
plain \verb|CUBE| --- an unknown word is tried as a program on disk before
...
These words open whole languages: \verb|EHBASIC| (Chapter~\ref{cha:basic}),
\shipif{lang:MSBASIC}{\texttt{MSBASIC} (Chapter~\ref{cha:msbasic}), }\verb|BBC|
(Chapter~\ref{cha:tube}), \verb|FORTH| (Chapter~\ref{cha:forth}),
\verb|LOGO| (Chapter~\ref{cha:logo}),
\verb|RX| (Chapter~\ref{cha:rx}) and
\verb|CPM|\shipif{chapter:09-cpm}{ (Chapter~\ref{cha:cpm})}.
...
\shipif{app:TINY}{\texttt{TINY} & a dungeon crawl \\}
\shipif{app:OPL2}{\texttt{OPL2} & the sound chip, all nine voices \\}
...
\shipif{app:BALLS}{\texttt{BALLS} & the sprites, all of them at once \\}
\shipif{app:ANSIDEMO}{\texttt{ANSIDEMO} & what JIM can draw \\}
\shipif{app:SEGDEMO}{\texttt{SEGDEMO} & two overlays sharing one address \\}
\bottomrule
\end{tabular}
\end{center}
...
other thing, which is why RX uses this form\shipif{lang:MSBASIC}{ --- and why
Microsoft BASIC runs every \texttt{*} command through it}.
