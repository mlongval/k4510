#!/bin/sh
# `!` at the prompt: the host's shell on the Tube (program 4), fitted on every
# machine since 2026-09-08: a command runs in the machine's current directory
# and the prompt comes back.
set -e
cd "$(dirname "$0")/.."
export K4510_NO_STARTUP=1
out=$(./test/headless rom/kernal.bin '!echo BANG$((6*7)) && pwd
~DIR
...
if [ -x "$MP_DIR/bin/mp" ] && command -v cc65 >/dev/null 2>&1; then
    ...
    printf 'program bad;\nbegin\n  x := ;\nend.\n' > fs/SELFHOST/BAD.PAS
    out=$(./test/headless rom/kernal.bin 'CD /SELFHOST
~PAS HELLO
...
echo "bangtest: OK (!cmd runs in the machine's directory, output on screen, prompt back$extra)"
